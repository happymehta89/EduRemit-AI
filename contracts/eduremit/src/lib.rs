...
impl EduRemitContract {
    ...
    pub fn link_student(env: Env, parent: Address, student: Address, allowance: i128) {
        parent.require_auth();
        env.storage().persistent().set(&DataKey::Parent(student.clone()), &parent);
        env.storage().persistent().set(&DataKey::Allowance(student.clone()), &allowance);
        if !env.storage().persistent().has(&DataKey::Balance(student.clone())) {
            env.storage().persistent().set(&DataKey::Balance(student.clone()), &0i128);
        }
    }
    ...
    pub fn deposit(env: Env, parent: Address, student: Address, amount: i128) {
        ...
        let linked_parent: Address = env.storage().persistent().get(&DataKey::Parent(student.clone())).expect("Student not linked to a parent");
        ...
        // Update balance
        let mut balance: i128 = env.storage().persistent().get(&DataKey::Balance(student.clone())).unwrap_or(0);
        balance += amount;
    ...
    pub fn withdraw(env: Env, student: Address, amount: i128) {
        ...
        let mut balance: i128 = env.storage().persistent().get(&DataKey::Balance(student.clone())).expect("No balance for student");
        let allowance: i128 = env.storage().persistent().get(&DataKey::Allowance(student.clone())).unwrap_or(0);
    ...
    pub fn get_student_allowance(env: Env, student: Address) -> i128 {
        env.storage().persistent().get(&DataKey::Allowance(student)).unwrap_or(0)
    }
